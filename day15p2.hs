...
data CellType = Wall | Space | Oxygen deriving (Enum, Eq, Show)

initialState :: PathState
initialState = ([], startProgram program [])

findAllPaths :: (CellType -> Bool) -> PathState -> [PathState]
findAllPaths cellIsValid (path, curWriter) =
  ...
  where findAllPathsAlongDirection curDir =
          ...
          then []
                   ...
                   newWriter = curWriter >>= contProgram [curDir]
                   newState = (newPath, newWriter)
                   progOutput = toEnum $
                                  fromJust $
                                    getLast $ W.execWriter newWriter
                   addPathIfValid = if cellIsValid progOutput
                                      then (:) newState
                                      else id in
               addPathIfValid $
                 case progOutput of
                   Wall   -> []
                   Space  -> findAllPaths cellIsValid newState
                   Oxygen -> []

findAllPathsToOxygen :: PathState -> [PathState]
findAllPathsToOxygen = findAllPaths (== Oxygen)

shortestPathToOxygen :: PathState
shortestPathToOxygen = head $
                         L.sortBy (comparing $ length . fst) $
                           findAllPathsToOxygen initialState
