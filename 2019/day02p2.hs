...
findAnswer :: Int -> Int
findAnswer possAnswer =
  let noun = possAnswer `quot` 100
      verb = possAnswer `mod` 100
      modifiedProgram = program A.// [(1, noun), (2, verb)]
      result = runProgram modifiedProgram 0 in
  case result of
    Just resultProg ->
      if ((resultProg A.! 0) == 19690720) then
        possAnswer
        else findAnswer (possAnswer+1)
    Nothing -> findAnswer (possAnswer+1)

main :: IO ()
main = print $ findAnswer 0