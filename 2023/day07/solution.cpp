...
num typeH(string S) {
    ...
    if (cnts.back().first == 5) return 6;
    if (cnts.back().first == 4) return 5;
    if (cnts.back().first == 3 && cnts[sz(cnts)-2].first == 2) return 4;
    if (cnts.back().first == 3) return 3;
    if (cnts.back().first == 2 && cnts[sz(cnts)-2].first == 2) return 2;
    if (cnts.back().first == 2) return 1;
    return 0;
}
