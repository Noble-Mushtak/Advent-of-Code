...
totalPath :: String
totalPath = "L,10,R,12,R,12,R,6,R,10,L,10,L,10,R,12,R,12,R,10,L,10,L,12,R,6,R,6,R,10,L,10,R,10,L,10,L,12,R,6,R,6,R,10,L,10,R,10,L,10,L,12,R,6,L,10,R,12,R,12,R,10,L,10,L,12,R,6"

aFunc :: String
aFunc = "R,10,L,10,L,12,R,6"

bFunc :: String
bFunc = "R,6,R,10,L,10"

cFunc :: String
cFunc = "L,10,R,12,R,12"

data Function = A | B | C deriving (Show)

allFuncs :: [Function]
allFuncs = [C, B, C, A, B, A, B, A, C, A]

mainRoutine :: String
mainRoutine = assert (totalPath == allFuncsPath) $
                L.intercalate "," $ map show allFuncs
  where parseFunction A = aFunc
        parseFunction B = bFunc
        parseFunction C = cFunc
        
        allFuncsPath = L.intercalate "," $ map parseFunction allFuncs

progInput :: String
progInput = L.intercalate "\n" [mainRoutine, aFunc, bFunc, cFunc, "n", ""]

progOutputs :: [Int]
progOutputs = runProgram program $ map fromEnum progInput

main :: IO ()
main = do
    putStrLn $ map toEnum progOutputs
    print $ last progOutputs