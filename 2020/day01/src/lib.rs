...
struct ExpenseReport { expenses: Vec<u64> }
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        Ok(ExpenseReport::new(
            s.trim()
             .split("\n")
             .map(|line| line.parse()
                             .map_err(|_| Self::Err::ParseLineError))
...
mod tests {
    ...
    fn test_parse_report() {
        ...
        assert_eq!("1721\n979\n366\n299\n".parse(), Ok(ExpenseReport::new(vec![1721, 979, 366, 299])));
    ...
    fn test_p1_answer() {
        ...
        assert!(calc_p1_answer(&ExpenseReport::new(vec![1780, 1693, 1830, 1756, 1858, 100])).is_err());
        assert_eq!(calc_p1_answer(&ExpenseReport::new(vec![1010])), Ok(1010*1010));
        assert_eq!(calc_p1_answer(&ExpenseReport::new(vec![1721, 979, 366, 299, 675, 1456])), Ok(1721*299));
    }
    ...
    fn test_p2_answer() {
        ...
        assert_eq!(calc_p2_answer(&ExpenseReport::new(vec![1721, 979, 366, 299, 675, 1456])), Ok(979*366*675));
    }
}