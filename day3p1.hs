...
pointsOfWire' :: Point -> Wire -> S.Set Point
pointsOfWire' startLoc ((curDir, curShift):restOfWire) =
  let lastPoint = incrementPoint startLoc curDir curShift
      calcNewPoint = incrementPoint startLoc curDir
      newPoints = S.fromList $ map calcNewPoint [1..curShift] in
  S.union newPoints $ pointsOfWire' lastPoint restOfWire
pointsOfWire' _ [] = S.empty

pointsOfWire :: Wire -> S.Set Point
pointsOfWire = pointsOfWire' (0, 0)

intersections :: S.Set Point
intersections = S.intersection (pointsOfWire wire1) (pointsOfWire wire2)

manhattanDistance :: Point -> Int
manhattanDistance (x, y) = (abs x)+(abs y)

main :: IO ()
main = print $ S.findMin $ S.map manhattanDistance intersections