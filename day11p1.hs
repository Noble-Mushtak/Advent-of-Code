...
type Panel = (Direction, Point, M.Map Point Int)

colorUnderPanel :: Panel -> Int
colorUnderPanel (_, curLoc, curMap) = M.findWithDefault 0 curLoc curMap

updatePanel :: Panel -> (Int, Int) -> Panel
updatePanel (curDir, curLoc, curMap) (color, direction) =
...
finalPanel :: Panel
finalPanel =
  let allPanels = L.scanl' updatePanel (UP, (0, 0), M.empty) $
                    curryList outputs
      inputs = map colorUnderPanel allPanels
      outputs = runProgram program inputs
  in last allPanels

numPainted :: Panel -> Int
numPainted (_, _, curMap) = M.size curMap

main :: IO ()
main = print $ numPainted finalPanel